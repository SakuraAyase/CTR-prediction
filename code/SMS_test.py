...
flags.DEFINE_integer('SMS_number', 2, 'SMS_numbers (>=0)')
flags.DEFINE_integer('choose_num1', 2, 'choose_num1 for the random generators (>=0)')
flags.DEFINE_integer('choose_num2', 2, 'choose_num2 for the random generators (>=0)')
flags.DEFINE_integer('choose_num3', 2, 'choose_num3 for the random generators (>=0)')
flags.DEFINE_integer('choose_num4', 2, 'choose_num4 for the random generators (>=0)')
flags.DEFINE_integer('choose_num5', 2, 'choose_num5 for the random generators (>=0)')
...
v = []
len_rd = 50
if FLAGS.seed>len_rd:
    print("Please input seed <",len_rd)
    sys.exit(1)
...
for ind in range(maxlen):
        t = v_best[ind]
        Best_map_.append(SliceM[t-1])
Best_mg = merge(Best_map_,mode='concat',concat_axis=1)
Best_map = Reshape((maxlen, embedding_dims, 1))(Best_mg)

# SG choose Map
# chosen number 1
SMS_number = FLAGS.SMS_number
SMS_maps = []
# SG choose Map
# chosen number 1
SM1 = []
vt1 = v[FLAGS.choose_num1]
for ind in range(maxlen):
        t = vt1[ind]
        SM1.append(SliceM[t-1])
SM1_mg = merge(SM1,mode='concat',concat_axis=1)
SM1_map = Reshape((maxlen, embedding_dims, 1))(SM1_mg)

SMS_maps.append(Best_map)
SMS_maps.append(SM1_map)

# chosen number 2
if SMS_number>2:
    SM2 = []
    vt2 = v[FLAGS.choose_num2]
    for ind in range(maxlen):
            t = vt2[ind]
            SM2.append(SliceM[t-1])
    SM2_mg = merge(SM2,mode='concat',concat_axis=1)
    SM2_map = Reshape((maxlen, embedding_dims, 1))(SM2_mg)
    SMS_maps.append(SM2_map)

# chosen number 3
if SMS_number>3:
    SM3 = []
    vt3 = v[FLAGS.choose_num3]
    for ind in range(maxlen):
            t = vt3[ind]
            SM3.append(SliceM[t-1])
    SM3_mg = merge(SM3,mode='concat',concat_axis=1)
    SM3_map = Reshape((maxlen, embedding_dims, 1))(SM3_mg)
    SMS_maps.append(SM3_map)

# chosen number 4
if SMS_number>4:
    SM4 = []
    vt4 = v[FLAGS.choose_num4]
    for ind in range(maxlen):
            t = vt4[ind]
            SM4.append(SliceM[t-1])
    SM4_mg = merge(SM4,mode='concat',concat_axis=1)
    SM4_map = Reshape((maxlen, embedding_dims, 1))(SM4_mg)
    SMS_maps.append(SM4_map)

# chosen number 5
if SMS_number>5:
    SM5 = []
    vt5 = v[FLAGS.choose_num5]
    for ind in range(maxlen):
            t = vt5[ind]
            SM5.append(SliceM[t-1])
    SM5_mg = merge(SM5,mode='concat',concat_axis=1)
    SM5_map = Reshape((maxlen, embedding_dims, 1))(SM5_mg)
    SMS_maps.append(SM5_map)
# Merge the stacked number
SMS_map_ = merge(SMS_maps,mode='concat')
SMS_map = Reshape((maxlen, embedding_dims, SMS_number))(SMS_map_)

##
convs = []
for index in range(embedding_dims):
        #print ("i:",index)
        t = Lambda(slice,output_shape=(maxlen,1,SMS_number),arguments={'index':index},
                   name='slice_'+str(index+1))(SMS_map)
        x = Reshape((maxlen,1,SMS_number))(t) #(batch, height, width, channels)
