...
w3 = 3
#计算每层pooling之后,剩余的元素
L1 = int(round((1-1.0/9)*maxlen))
L2 = int(round((1-2.0/3)*maxlen))
L3 = 3
...
for index in range(embedding_dims):
        ...
        x = MaxPooling2D(pool_size=(pool1,1),strides=(1,1),
                         border_mode='valid',dim_ordering='tf')(x)
        #x = Activation('tanh')(x)
        x = Activation('sigmoid')(x)

        #第二层conv and pooling
        x = convolutional.ZeroPadding2D(padding=(w2-1,0))(x)
        #卷积2后的尺寸conv2
        conv2 = L1+w2-1
        ...
        x = MaxPooling2D(pool_size=(pool2,1),strides=(1,1),
                         border_mode='valid',dim_ordering='tf')(x)
        #x = Activation('tanh')(x)
        x = Activation('sigmoid')(x)        
...
for epoch in range(num_epoch):
	if epoch==0:
		model.fit_generator(generate_arrays_from_file('Data/train_hash.csv',batch_size=batch_size),steps_per_epoch=steps_per_epoch,epochs=10,max_queue_size=max_q_size,verbose=1)
